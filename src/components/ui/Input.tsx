  ...
  ({ className, label, error, icon, type, ...props }, ref) => {
    ...
    return (
      <div className="w-full">
        {label && (
          <label htmlFor={id} className="block text-sm font-medium text-navy-900 dark:text-white mb-1.5">
            {label}
          </label>
        )}
        <div className="relative">
          ...
          <input
            ...
            className={cn(
              'w-full px-4 py-2.5 rounded-sm border bg-white text-navy-900 placeholder:text-navy-400',
              'focus:outline-none focus:ring-2 focus:ring-gold-500/50 focus:border-gold-500',
              'transition-all duration-200',
              'dark:bg-navy-900 dark:text-white dark:border-navy-700 dark:placeholder:text-navy-500',
              icon && 'pl-10',
              isPassword && 'pr-10',
